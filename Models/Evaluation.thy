...
lemma "\<iota> X \<parallel> \<iota> X = \<iota> X"
  apply (auto simp add: pow_inv_def shuffle_def tshuffle_words_def image_def)
...
definition steps :: "'a rel llist \<Rightarrow> 'a set \<Rightarrow> enat" where
  "steps xs H \<equiv> inf (llength xs) (Inf {enat n|n. eval_finite_word (list_of (ltake (enat n) xs)) H = {}})"
...
lemma steps_LNil: "steps LNil H = 0"
  by (simp add: steps_def) (metis bot_enat_def inf_bot_left)

lemma steps_empty: "steps xs {} = 0"
...
lemma steps_mono: "P \<subseteq> Q \<Longrightarrow> steps xs P \<le> steps xs Q"
  by (auto intro!: order_trans[OF inf_sup_ord(2)] Inf_superset_mono simp add: steps_def eval_finite_word_def)

lemma eval_finite_word_mono: "P \<subseteq> Q \<Longrightarrow> eval_finite_word xs P \<le> eval_finite_word xs Q"
...
proof (cases "llength xs \<le> Inf {enat n |n. eval_finite_word (list_of (ltake (enat n) xs)) H = {}}")
  ...
  hence "steps xs H = llength xs"
    by (simp add: steps_def inf_absorb1[OF c])
  thus ?thesis
    by (metis assms)
next
  case False
  ...
  hence "Inf {enat n |n. eval_finite_word (list_of (ltake (enat n) xs)) H = {}} = enat n"
    by (simp add: steps_def inf_absorb2[OF c] assms(1)[symmetric])
