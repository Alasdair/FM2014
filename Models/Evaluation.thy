...
lemma mod_isol: "p \<le> q \<Longrightarrow> x \<Colon> p \<le> x \<Colon> q"
  ...
  apply (rule Sup_mono)
  apply (auto simp add: top_top_def[symmetric])
  apply (cases p)
  apply auto
  apply (rule Sup_mono)
  apply auto
...
lemma NotTop_continuous: "NotTop (\<Union>X) = Sup (NotTop ` X)"
  by (auto simp add: Sup_top_def Top_top) (metis top.distinct(1) top_top_def)
...
lemma [simp]: "(top :: 'a::complete_lattice top) = bot \<longleftrightarrow> False"
  by (simp add: top_top_def bot_top_def)
...
proof -
  ...
  also have "... \<le> eval_word w \<hookleftarrow> Sup X"
    by (auto simp add: Sup_top_def Top_top) (metis bot_least bot_top_def)
  finally show ?thesis .
qed
...
lemma mod2_isol: "x \<le> y \<Longrightarrow> x \<triangleright> p \<le> y \<triangleright> p"
  by (cases p) (auto intro: Sup_mono)

thm mod_isor

lemma mod2_isor: "p \<le> q \<Longrightarrow> x \<triangleright> p \<le> x \<triangleright> q"
  apply (cases q)
  apply simp
  apply (cases p)
  apply simp
