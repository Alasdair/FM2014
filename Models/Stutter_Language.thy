...
lemma ldropWhile_is_right: "\<ll> t \<noteq> LNil \<Longrightarrow> ldropWhile is_right t = LCons (Inl ()) (ltl (ldropWhile is_right t))"
  ...
  apply (erule bexE)
  apply (drule split_llist_first)
  apply (erule exE)+
  apply simp
  apply (drule is_leftD)
  apply (erule exE)
  apply simp
  apply (subgoal_tac "ldropWhile is_right ys = LNil")
  apply (metis (hide_lams, mono_tags) is_right.simps(2) ldropWhile_LCons ldropWhile_LNil_lappend ltl_simps(2))
  by (metis (full_types) no_lefts_ldropWhile_is_right)
...
proof -
...

  {
    ...
    proof (induct arbitrary: t rule: lfinite_induct)
      case (Nil t)
      thus ?case
        by simp (metis (full_types) LNil_eq_llist_map lfilter_LCons llist.case_conv_if llist.collapse ltakeWhile_eq_LNil_iff ltl_def)
    next
      case (Cons x xs t)
      then obtain r and t' where [simp]: "t = LCons (Inr r) t'"
        by (metis is_right.simps(2) llist.discI ltakeWhile_LCons ltakeWhile_LNil sumlist_cases)
...
lemma drop_rights: "\<ll> t = LCons x xs \<Longrightarrow> ldropWhile is_right t = LCons (Inl x) (ltl (ldropWhile is_right t))"
  ...
  apply auto
  apply (metis in_lset_ldropWhileD lfilter_eq_LCons lhd_LCons lhd_in_lset llist.distinct(1))
