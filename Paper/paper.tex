...
The shuffle product $\sha$ on a pair of finite words is defined inductively as
\begin{align*}
\epsilon \sha s &= \{s\},\\
s \sha \epsilon &= \{s\},\\
as \sha bt &= a(s \sha bt) \cup b(as \sha t).
\end{align*}
Shuffle is associative, commutative, and distributes over arbitrary
joins.

\begin{lem}
$(\mathcal{P}((\Sigma\times\Sigma)^*), \cup, \cdot, \sha, \emptyset, \{\epsilon\})$ is a weak trioid.
\end{lem}
