...
\begin{prooftree}
\RightLabel{\ Parallel}
\AxiomC{$r_1, g_1 \vdash \{p_1\}x\{q_1\}$}
\AxiomC{$g_1 \le r_2$}
\AxiomC{$r_2, g_2 \vdash \{p_2\}y\{q_2\}$}
\AxiomC{$g_2 \le r_1$}
\QuaternaryInfC{$r_1 \sqcap r_2, g_1 \| g_2 \vdash \{p_1 \sqcap p_2\}x\|y\{q_1 \sqcap q_2\}$}
\end{prooftree}
...
consistent. This word is highlighted with solid arrows in the diagram
above. Now if we shuffle the single state pair $(\sigma_2, \sigma_3)$
into the above language, would end up with a language containing the
words represented in the diagram below:
